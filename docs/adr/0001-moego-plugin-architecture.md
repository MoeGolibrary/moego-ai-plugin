...
创建独立 `MoeGolibrary/moego-ai-marketplace` 仓库：
...
- **Marketplace 冷启动**：`moego-ai-marketplace` repo 需额外创建和维护；未纳入官方 Marketplace 前，安装命令对新成员仍需文档引导
...
- **Marketplace 可用性**：`MoeGolibrary/moego-ai-marketplace` 成为分发关键路径，仓库宕机影响安装。**缓解**：在 README 中同时提供 fallback 安装方式（直接 `git clone https://github.com/MoeGolibrary/moego-ai-plugin`）。
...
| Phase 3 | 创建 `MoeGolibrary/moego-ai-marketplace` repo；更新团队安装文档 | 近期 |
