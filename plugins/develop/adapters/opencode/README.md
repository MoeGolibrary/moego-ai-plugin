...
OpenCode 直接读取 SKILL.md 的 `name` 字段。Develop Plugin 使用 bare name，需通过 adapter 脚本创建带 `develop-` 前缀的符号链接。
...
for skill_dir in "$PLUGIN_ROOT"/skills/*/; do
  ...
  [ -f "$skill_dir/SKILL.md" ] && ln -sfn "${skill_dir%/}" ~/.config/opencode/skills/develop-${skill}
done
```
